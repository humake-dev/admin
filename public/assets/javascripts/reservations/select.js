...
function getList(current_page, jq) {
  ...
  if($('input[name="other_user"]').length) {
    if($('input[name="other_user"]:checked').val()==1) {
      s_param.other_user=true;
    }
  }

  if($("#rp_course_id").length) {
    s_param.course_id=$("#rp_course_id").val();
  }

  if($('#rp_trainer_id').length) {
    s_param.trainer_id=$("#rp_trainer_id").val();
  }

  $.getJSON('/reservations/select',s_param,function(data) {
      if(data.result=='success') {
        ...
        if(data.total) {
          ...
          $('.summary strong.mark').text(data.total+'명');
