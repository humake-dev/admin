...
function getList(current_page, jq) {
  ...
  $.getJSON('/user-not-counsel-selects'+qq,s_param,function(data) {
      if(data.result=='success') {
        ...
        if(data.total) {
          $.each(data.list,function(index,value){
              ...
              if(value.user_id) {
                var input='<td><input name="id[]" value="'+value.user_id+'" type="checkbox"></td>';
              } else {
                var input='<td><input name="temp_user_id[]" value="'+value.temp_user_id+'" type="checkbox"></td>';
              }
            ...
            if(value.card_no) {
              var card_no=value.card_no;
            } else {
              var card_no='없음'
            }

            $("#user_select_list tbody").append('<tr>'+input+'<td class="name">'+value.name+'</td><td>'+card_no+'</td><td>'+birthday+'</td><td>'+display_gender(value['gender'])+'</td><td class="phone">'+phone+'</td></tr>');
          });
...
function m_input_change() {
  if(multi) {
    ...
    if($(this).attr('name')=='temp_user_id[]') {
      sync_selct_temp_user($(this).val());
    } else {
...
function sync_selct_temp_user(temp_user_id) {
  var i_input=$('#user_select_list input[value="'+temp_user_id+'"]');
  var tr=$('#user_select_list input[value="'+temp_user_id+'"]').parent().parent();

  if($('.not_users_input input[value="'+temp_user_id+'"]').length) {
    $('.not_users_input input[value="'+temp_user_id+'"]').parent().remove();
    i_input.prop('checked',false);
    tr.removeClass('table-primary');
    return false;
  }
  ...
  var span=$('<span class="select_user text-success">'+name+'<input type="hidden" name="not_temp_user[]" value="'+temp_user_id+'" /> <span class="text-danger">X<span></span>');
  span.find('.text-danger').click(deleteSelectedUser);
  $('.not_users_input').append(span);
}
