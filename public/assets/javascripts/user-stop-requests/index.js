$(function () {
  ...
  $('#user_stop_request_search .card-header').click(function(){
    if($(this).find('.buttons i').text()=='keyboard_arrow_down') {
      $.post('/user-stop-requests/index-oc',{'format':'json'},function(){
      ...
      if(index) {
        $.post('/user-stop-requests/index-oc/field',{'format':'json'},function(){
  
        },'json');
      } else {
          $.post('/user-stop-requests/index-oc/default',{'format':'json'},function(){
  ...
  $("#user_stop_request_search .card-header .nav-item .nav-link").click(function(event){
