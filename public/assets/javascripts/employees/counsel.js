function getList(current_page, jq) {
    ...
    $.getJSON('',{'format':'json','per_page':perPage,'page':pageID},function(data) {
        if(data.result=='success') {
          $("#employee_counsel_table tbody").empty();
          $('#employee_counsel_count').val(data.total);
  
          if(data.total) {
            $.each(data.list,function(index,value){
              var number=data.total-(perPage*(pageID-1))-index;
  
              var execute_date=value.execute_date;
              
              employee_id=$("#form_photo").attr('action').split('update-photo/')[1];

              if (value.complete=='1') {
                var process='<span class="text-success">'+$('#processing_complete_text').text()+'</span>';
              } else {
                var process='<span>'+$('#processing_text').text()+'</span>';
              }

              switch(value.type) {
                case 'E' : 
                  var c_type=$('#counsel_interview_text').text();
                  break;
                default : 
                  var c_type=$('#counsel_phone_text').text();
              }
              ...
              $("#employee_counsel_table tbody").append('<tr><td>'+number+'</td><td><a href="/view/'+value['id']+'" target="_blank">'+value['name']+' / '+phone+'</a></td><td class="text-right">'+process+'</td><td class="text-right">'+c_type+'</td><td class="text-right">'+execute_date+'</td><td class="text-right"><label style="display:block"><input type="checkbox" name="user_id[]" value="'+value['id']+'"'+checked+'></label></td></tr>');
            });
  
            $('#employee_counsel_table input[name="user_id[]"]').change(eu_select_change);
            $("#employee_counsel_table tr").css('cursor','pointer');
  
            if($("#employee_counsel_table tbody input:checkbox:not(:checked)").length) {
            ...
            $("#employee_counsel_table tbody").append('<tr><td colspan="2" style="text-align:center">해당 데이터가 없습니다.</td></tr>');
          }
  ...
  $(function () {
    ...
    $('#select-oe').change(function() {
      $('#eu_unselect,#eu_select').empty();
        ...
        if($(this).val()=='') {
          $("#check_all").prop('checked',false);
          $('#employee_counsel_table tbody input[type="checkbox"]').prop('checked',false);
          ...
          if($('#select-oe').val()==result) {
            $('#employee_counsel_table tbody input[type="checkbox"]').prop('checked',true);
              $("#check_all").prop('checked',true);
          } else {
            $('#employee_counsel_table tbody input[type="checkbox"]').prop('checked',false);
    ...
    check_checked('employee_counsel_table');  
    ...
    if($("#employee_counsel_table").length) {
      initPagination(Number($('#employee_counsel_count').val()),10);
    }
