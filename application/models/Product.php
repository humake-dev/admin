...
class Product extends SL_Model
{
    ...
    {
        $this->pdo->select('p.*,p.id as product_id,pc.title as category_name,GROUP_CONCAT(CONCAT(pp.id,"::",pp.picture_url)) as picture_url');
        ...
        if (empty($this->type)) {
            $this->pdo->where(array('p.branch_id' => $this->branch_id, 'p.enable' => true, 'pc.enable' => true));
        } else {
            if ($this->type == 'admin') {
    ...
    {
        $this->pdo->select('p.id,p.id as product_id,p.title,p.product_category_id,p.price,pc.title as category_name,GROUP_CONCAT(CONCAT(pp.id,"::",pp.picture_url)) as picture_url');
        $this->pdo->join('product_pictures as pp', 'pp.product_id=p.id', 'left');
        $this->pdo->join('product_categories as pc', 'p.product_category_id=pc.id', 'left');

        if (empty($this->type) and !in_array($id,array(4,10))) {
            $this->pdo->where(array('p.id' => $id, 'p.enable' => 1, 'p.branch_id' => $this->branch_id));
        } else {
