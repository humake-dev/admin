...
try {
    ...
    $branch_id=5;
    $left_fcs=array(1577,1589);

    $fcs_count=count($left_fcs);
    ...
    $stmt_count_etc = $pdo->prepare('SELECT count(*) FROM users as u INNER JOIN user_fcs as uf ON uf.user_id=u.id WHERE u.branch_id=:branch_id');
    ...
    if (!empty($etc_count)) {
        $stmt_select_etc = $pdo->prepare('SELECT u.* FROM users as u INNER JOIN user_fcs as uf ON uf.user_id=u.id WHERE u.branch_id=:branch_id');
