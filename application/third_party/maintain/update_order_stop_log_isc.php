...
try {
    ...
    $stmt_count = $pdo->prepare('SELECT count(*) FROM order_stop_logs AS osl INNER JOIN orders AS o ON osl.order_id=o.id WHERE o.branch_id=:branch_id');
    ...
    $stmt_select = $pdo->prepare('SELECT osl.* FROM order_stop_logs AS osl INNER JOIN orders AS o ON osl.order_id=o.id WHERE o.branch_id=:branch_id');
    $stmt_select->bindParam(':branch_id', $branch_id, PDO::PARAM_INT);
    $stmt_select->execute();
    $lists = $stmt_select->fetchAll(PDO::FETCH_ASSOC);
    $stmt_select->closeCursor();

    $stmt_update = $pdo->prepare('UPDATE order_stop_logs SET stop_day_count=:stop_day_count WHERE id=:id');
    ...
    foreach ($lists as $value) {
        ...
        sl_log($log, 'order_stop_log_id :'.$value['id'].' updated '.$value['id'].' to '.$stop_day_count);
    }
