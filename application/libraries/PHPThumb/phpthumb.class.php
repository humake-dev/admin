...
$_SERVER['PHP_SELF'] = (isset($_SERVER['PHP_SELF']) ? $_SERVER['PHP_SELF'] : ''); // initialize it once here if undefined since it is referred to several times elsewhere
...
//echo $maxwidth.'x'.$maxheight.'<hr>';
	...
	public function ExtractEXIFgetImageSize() {
		...
		$this->getimagesizeinfo[2] = (isset($this->getimagesizeinfo[2]) ? $this->getimagesizeinfo[2] : '');
...
//				return $this->ErrorImage('Unknown image type identified by "'.substr($this->rawImageData, 0, 4).'" ('.phpthumb_functions::HexCharDisplay(substr($this->rawImageData, 0, 4)).') in SourceImageToGD()['.__LINE__.']');
	...
	public function phpThumbDebug($level='') {
		...
		$DebugOutput[] = '$_SERVER[SERVER_SOFTWARE]   = '.(isset($_SERVER['SERVER_SOFTWARE']) ? $_SERVER['SERVER_SOFTWARE'] : '');
		$DebugOutput[] = '__FILE__                    = '.__FILE__;
		$DebugOutput[] = 'realpath(.)                 = '.@realpath('.');
		$DebugOutput[] = '$_SERVER[PHP_SELF]          = '.$_SERVER['PHP_SELF'];
		$DebugOutput[] = '$_SERVER[HOST_NAME]         = '.(isset($_SERVER['HOST_NAME'])       ? $_SERVER['HOST_NAME'] : '');
		$DebugOutput[] = '$_SERVER[HTTP_REFERER]      = '.(isset($_SERVER['HTTP_REFERER'])    ? $_SERVER['HTTP_REFERER'] : '');
		$DebugOutput[] = '$_SERVER[QUERY_STRING]      = '.(isset($_SERVER['QUERY_STRING'])    ? $_SERVER['QUERY_STRING'] : '');
		$DebugOutput[] = '$_SERVER[PATH_INFO]         = '.(isset($_SERVER['PATH_INFO'])       ? $_SERVER['PATH_INFO'] : '');
		$DebugOutput[] = '$_SERVER[DOCUMENT_ROOT]     = '.(isset($_SERVER['DOCUMENT_ROOT'])   ? $_SERVER['DOCUMENT_ROOT']  : '');
