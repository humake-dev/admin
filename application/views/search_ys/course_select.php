        ...
        if ($search_data['course_category']['total']):   
            foreach ($search_data['course_category']['list'] as $course_category):
                ...
                <optgroup label="<?php echo $course_category['title']; ?>">
                    <?php if ($search_data['course']['total']):
                        $pt_list = array();
                        foreach ($search_data['course']['list'] as $course):
                ...
                <?php endif; ?>                
            <?php endforeach; ?>
            <optgroup label="양도수수료">
                <option value="4" <?php if (isset($search_data['product_id'])): ?><?php if (in_array(4, $search_data['product_id'])): ?> selected="selected"<?php endif; ?><?php endif; ?>>양도수수료</option>
            </optgroup>
        <?php endif; ?>
    </select>
