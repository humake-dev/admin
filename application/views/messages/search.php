<div style="width:100%">
  <nav class="sub_nav">
    <ul>
      <li <?php if ($type=='event'): ?>class="curr"<?php endif ?>><a href="search.php?type=event"><?php echo _('Search by Period') ?></a></li>
      <li <?php if ($type=='status'): ?>class="curr"<?php endif ?>><a href="search.php?type=status"><?php echo _('Search by Status') ?></a></li>
      <li <?php if ($type=='field'): ?>class="curr"<?php endif ?>><a href="search.php?type=field"><?php echo _('Search by Item') ?></a></li>
    </ul>
  </nav>
  <?php include __DIR__.DIRECTORY_SEPARATOR.'..'.DIRECTORY_SEPARATOR.'inc'.DIRECTORY_SEPARATOR.'message.php' ?>
  <article class="regist_w">
    <div class="title_a">
      <h1 class="title"><?php echo _('Search User') ?></h1>
    </div>
    ...
    <?php if ($type=='event'): ?>
        ...
        <div class="row">
            ...
            <div style="float:left;position:relative;width:165px">
              <input type="text" id="start_date" name="start_date" class="datepicker"<?php if (isset($display_start_date)): ?> value="<?php echo $display_start_date ?>"<?php endif ?> />
            </div>
            <span style="display:block;float:left">~</span>
            <div style="float:left;position:relative;width:165px">
              <input type="text" id="end_date" name="end_date" class="datepicker"<?php if (isset($display_end_date)): ?> value="<?php echo $display_end_date ?>"<?php endif ?> />
            </div>
            <div style="float:left;margin-left:50px">
              <label><input type="radio" name="date_p" value="0"<?php if ($date_p==0): ?> checked="checked"<?php endif ?> /><?php echo _('Today') ?></label>
    					<label><input type="radio" name="date_p" value="7"<?php if ($date_p==7): ?> checked="checked"<?php endif ?> /><?php echo _('Period Week') ?></label>
    					<label><input type="radio" name="date_p" value="30"<?php if ($date_p==30): ?> checked="checked"<?php endif ?> /<?php echo _('A month') ?>></label>
    					<label><input type="radio" name="date_p" value="90"<?php if ($date_p==90): ?> checked="checked"<?php endif ?> /<?php echo _('Three month') ?>></label>
    					<label><input type="radio" name="date_p" value="180"<?php if ($date_p==180): ?> checked="checked"<?php endif ?> /><?php echo _('Half a year') ?></label>
    					<label><input type="radio" name="date_p" value="365"<?php if ($date_p==365): ?> checked="checked"<?php endif ?> /><?php echo _('One Year') ?></label>
    					<label><input type="radio" name="date_p" value="all"<?php if (!strcmp($date_p, 'all')): ?> checked="checked"<?php endif ?> /><?php echo _('The whole period') ?></label>
            </div>
        </div>
        <input type="submit" class="btn_search" value="검색" /><?php if ($search): ?><a href="search.php" style="margin-left:20px"><?php echo _('Turn off Search') ?></a><?php endif ?>
    <?php elseif ($type=='status'): ?>
      <div class="row">
        <label><input type="radio" name="status_type" value="course"<?php if ($status_type=='course'): ?> checked="checked"<?php endif ?> /><?php echo _('By course') ?></label>
        <label><input type="radio" name="status_type" value="payment"<?php if ($status_type=='payment'): ?> checked="checked"<?php endif ?> /><?php echo _('By payment status') ?></label>
      </div>
      <div class="row">
      <select id="status_course_select" name="course_id" style="margin-right:20px;<?php if ($status_type=='payment'): ?>display:none;<?php endif ?>"  <?php if ($status_type=='payment'): ?>disabled="disabled"<?php endif ?>>
        <?php if ($status_count): ?>
      ...
      <select id="status_payment_select" name="payment_id" style="margin-right:20px;<?php if ($status_type=='course'): ?>display:none;<?php endif ?>" <?php if ($status_type=='course'): ?>disabled="disabled"<?php endif ?>>
        ...
        <input type="submit" class="btn_search" value="검색" /><?php if ($search): ?><a href="search.php?type=status" style="margin-left:20px"><?php echo _('Turn off Search') ?></a><?php endif ?>
      </div>
    <?php else: ?>
        ...
        <input type="text" id="search_word" name="search_word"<?php if ($search): ?> value="<?php echo $search_word ?>"<?php endif ?> required="required" />
        <input type="submit" class="btn_search" value="검색" /><?php if ($search): ?><a href="search.php?type=field" style="margin-left:20px"><?php echo _('Turn off Search') ?></a><?php endif ?>
      </div>
  <?php endif ?>
  </div>
  ...
  <article class="list_w">
    <div class="sc_box" id="member_list">
              <table class="standard box nowrap" style="margin-top:20px">
                  ...
                  <thead>
                      <tr>
                          ...
                          <th><?php echo _('Total PT Count') ?>/<?php echo _('Use Quantity') ?>/<?php echo _('Remaining number') ?></th>
                  ...
                  <tbody>
                    ...
                    <?php foreach ($data['list'] as $index=>$value): ?>
                      <tr>
                          ...
                          <td><?php echo change_gender($value['gender']) ?></td>
                    ...
                    <?php else: ?>
                      <tr>
                        <td colspan="13"><?php echo _('StatNo such member') ?>.</td>
                      </tr>
                    <?php endif ?>
                  </tbody>
              </table>
        ...
        <ul style="float:left;width:100%;padding:10px 0 20px">
          <li><a href="/message/new.php?type=sms" target="_blank" class="btn_popup" title="메세지 보내기" style="margin-left:0"><img src="/resource/images/common/btn_file_download.png" alt=""><?php echo _('Send SMS') ?></a></li>
          <li><a href="/message/new.php?type=push" target="_blank" class="btn_popup" title="메세지 보내기" style="margin-left:10px"><img src="/resource/images/common/btn_file_download.png" alt=""><?php echo _('Send Push') ?></a></li>
        </ul>
    </article>
</div>
