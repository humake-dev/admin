...
class Counsel_requests extends SL_Controller
{
    use Search_period;

    protected $model = 'CounselRequest';
    protected $permission_controller = 'counsels';
    protected $script = 'counsel-requests/index.js';
    ...
    public function index_oc($type = null)
    {
        if (in_array($type, ['default', 'field'])) {
            $this->session->set_userdata('counsel_request_search_open', $type);
        } else {
            $this->session->unset_userdata('counsel_request_search_open');
        }
